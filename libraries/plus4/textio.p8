...
sub colorx (ubyte txtcol) {
    ;txtcol &= 15
    ; could also use chrout() of the correct character?
    ;cbm.COLOR = plus4.color_table[txtcol]
    cbm.COLOR = plus4.color_table[txtcol & 15]
}
...
_colorrows	.word  cbm.Colors + range(0, 1000, 40)
        ; !notreached!
	}}
}

asmsub  setclr2  (ubyte col @X, ubyte row @Y, ubyte color @A) clobbers(A, Y)  {
	; ---- set the Plus/4 color (luma+chroma) in A on the screen matrix at the given position
	%asm {{
		pha
		tya
		asl  a
...
asmsub  getclr  (ubyte col @A, ubyte row @Y) clobbers(Y) -> ubyte @ A {
	...
	%asm  {{
		...
		inc  _mod+2
_mod		lda  $ffff		; modified
                and  #%00001111         ; mask off luminance
		rts
	}}
}
