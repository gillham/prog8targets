...
txt {
    ...
    ; 8x8 with 3x4 font (4x4 with gap) = 32 * 32 pixels.
    const ubyte DEFAULT_WIDTH = 8
    const ubyte DEFAULT_HEIGHT = 8
...
sub  clear_screen() {
    sys.set_irqd()
    sys.memset(mc64.Screen, 8*8, 32)
    sys.memset(mc64.Color, 8*8, 32)
    sys.memset(mc64.Bitmap, 1024, 0)
    sys.clear_irqd()
    vcolor = 63
}
...
+           lda  #>MYFONT2  ; setup pointer to 3x4 font
            ...
            lda  txt.trow           ; load row number
            asl                     ; multiply by two
            tay                     ; put in Y index
            lda  _bitmaprows,y      ; low byte of this row address
            sta  SPTR               ; save in low byte of screen pointer
            lda  _bitmaprows+1,y    ; high byte of this row address
            sta  SPTR+1             ; save in high byte of screen pointer
            lda  txt.tcol           ; load column and multiply by 4 (font width)
            asl
            asl
            clc
            adc  SPTR               ; add column offset to screen pointer
            sta  SPTR               ; save back to screen pointer low byte
            bcc  +
            inc  SPTR+1             ; low byte wrapped so increment high byte
+           nop

;            lda  #<mc64.Bitmap  ; low byte of screen memory start
;            clc                 ; row offset is 32 pixel * 4 pixel (row width * font height) = 128 (1/2page)
...
+           pla
            ...
            tay
            rts
;_bitmaprows	.word  mc64.Bitmap + range(0, DEFAULT_HEIGHT*4*DEFAULT_WIDTH*4, DEFAULT_WIDTH*4)
_bitmaprows	.word  mc64.Bitmap + range(0, 32*32, DEFAULT_WIDTH*4*4)
            ; !notreached!
        }}
    }
