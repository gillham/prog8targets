...
;
; Called by the custom launcher so we can have a minimal
; init_system startup and cleanup_at_exit.
;

p8_sys_startup {
    asmsub  init_system()  {
        ; Initial entry point for the program.
        ; Currently calls main.start() and returns zero to cx16os
        ; return code can be overridden with sys.exit()
        %asm {{
    ...
    asmsub  init_system_phase2()  {
        %asm {{
            rts     ; no phase 2 steps on CX16 OS
        }}
    }
